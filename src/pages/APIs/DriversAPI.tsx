...
const DriversAPI: React.FC = () => {
  ...
  const [updateSponsor, setUpdateSponsor] = useState("");
  ...
  const handleUpdateDriver = async () => {
	try {
	  ...
	  if (updateSponsor.trim() !== "") requestBody.DriverSponsor = updateSponsor;
	  if (updatePoints.trim() !== "") requestBody.DriverPoints = updatePoints;
		...
		<section>
		  <div className="space-y-2">
			...
			<input
			  ...
			  value={updateSponsor}
			  onChange={(e) => setUpdateSponsor(e.target.value)}
			  className="border border-gray-300 p-2 w-full rounded"
			/>
			<button
