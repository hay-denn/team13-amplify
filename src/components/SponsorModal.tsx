...
const API_SPONSOR_ORG_URL =
  "https://br9regxcob.execute-api.us-east-1.amazonaws.com/dev1";
const API_DRIVER_SPONSOR_APP_URL =
...
async function getDriverSponsors(driverEmail: string) {
  try {
    const response = await fetch(
      `${DRIVER_SPONSOR_URL}/driverssponsors?DriversEmail=${driverEmail}`
    );
...
async function manageCognitoUser(
  ...
  try {
    const response = await fetch(
      "https://7auyafrla5.execute-api.us-east-1.amazonaws.com/dev1/manage-user",
      {
        method: "POST",
        headers: {
...
async function callAPI(
  ...
  try {
    ...
    if (response.ok) {
      ...
      alert(
        "Unable to make user edit - Error: " +
          response.status +
          " - " +
          response.statusText
      ); // Display error alert with status and message
    }
  } catch (error) {
...
const userTypes = ["Sponsor", "Driver"];

const SponsorModal: React.FC<ModalProps> = ({
  isOpen,
  onClose,
  initialData,
  emailList,
    ...
    if (initialData) {
      ...
      if (initialData.org) {
        ...
        const matchedOrg = orgs.find(
          (org) => org.OrganizationName === initialData.org
        );
      ...
      if (!auth.user?.access_token) {
        ...
        if (!demoMode) {
          await manageCognitoUser(
            ...
            { given_name: firstName, family_name: familyName, email: email },
            "",
            userType
          );
        } else {
          alert(
            "Admin account deletion is not allowed from Manage Users page."
          );
        }
        if (userType == "Driver") {
          const data = {};
    ...
    if (!auth.user?.access_token) {
      ...
      if (newUser) {
        ...
        if (userType == "Driver") {
          ...
          await callAPI(`${DRIVER_URL}/driver`, "POST", data);
          ...
          await callAPI(`${DRIVER_SPONSOR_URL}/driverssponsor`, "POST", data2);
        } else if (userType == "Sponsor") {
          const data = {
            UserEmail: email,
            UserFName: firstName,
...
export const SponsorApplyModal = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${API_DRIVER_SPONSOR_APP_URL}/driversponsorapplication`,
        {
          method: "POST",
...
export const ViewOrgModal: React.FC<ViewOrgProps> = ({
  isOpen,
  onClose,
  email,
}) => {
  ...
  const availableOrgs = orgs.filter(
    (org) => !driverOrgIDs.has(org.OrganizationID)
  );
      ...
      <div className="modal-content">
        ...
        <ul className="modal-list">
          {driverOrgs.map((driverOrg) => {
            const org = orgs.find(
              (o) => o.OrganizationID === driverOrg.DriversSponsorID
            );
            return org ? (
